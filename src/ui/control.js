...
function createBrowserBridge() {
  const demo = { timer: { status: 'idle', phase: 'focus', remainingMs: 25 * 60_000, todayCount: 0 }, alarms: [], offwork: { enabled: true, time: '18:30', weekdays: [1, 2, 3, 4, 5], snoozeMinutes: 15, escalateMinutes: 15, allowAnnoyed: true }, settings: { preset: '25/5', customFocus: 25, customBreak: 5, voiceMode: 'key', volume: .75, interactions: true, launchAtLogin: false } };
  return { getState: async () => demo, command: async (name, payload) => { if (name === 'timer:start') { demo.timer.status = 'running'; demo.timer.remainingMs = payload.focusMinutes * 60_000; } return demo; }, onState: () => {}, showControl: () => {} };
}
...
function render(next) {
  ...
  $('#customTime').classList.toggle('hidden', selectedPreset !== 'custom'); renderAlarms(); renderSettings();
}

function renderAlarms() {
  $('#alarmList').innerHTML = state.alarms.length ? state.alarms.map((alarm) => '<article class="alarm-item"><input class="alarm-enabled" data-id="' + alarm.id + '" type="checkbox" ' + (alarm.enabled ? 'checked' : '') + '><div class="alarm-meta"><strong>' + escapeHtml(alarm.label) + '</strong><small>' + recurrence(alarm) + '</small></div><div class="alarm-tools"><button data-edit="' + alarm.id + '">编辑</button><button data-delete="' + alarm.id + '">删除</button></div></article>').join('') : '<div class="empty">还没有闹钟。末末目前只负责准时可爱。</div>';
...
function renderSettings() {
  const off = state.offwork; $('#offworkEnabled').checked = off.enabled; $('#offworkTime').value = off.time; $('#escalateMinutes').value = off.escalateMinutes; $('#offworkSnooze').value = off.snoozeMinutes;
...
$('#mainAction').onclick = () => { const t = state.timer; if (t.status === 'running') return command('timer:pause'); if (t.status === 'paused') return command('timer:resume'); const preset = selectedPreset === '50/10' ? [50, 10] : selectedPreset === 'custom' ? [Number($('#focusMinutes').value), Number($('#breakMinutes').value)] : [25, 5]; return command('timer:start', { task: $('#task').value, focusMinutes: preset[0], breakMinutes: preset[1] }); };
...
$$('[data-interaction]').forEach((b) => b.onclick = () => command('interaction', { kind: b.dataset.interaction })); $('#showPet').onclick = () => command('pet:visible', { visible: true });
...
$('#saveOffwork').onclick = () => command('offwork:update', { enabled: $('#offworkEnabled').checked, time: $('#offworkTime').value, escalateMinutes: Number($('#escalateMinutes').value), snoozeMinutes: Number($('#offworkSnooze').value), weekdays: $$('#offworkDays .on').map((b) => Number(b.dataset.day)) });
