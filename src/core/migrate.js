...
export const migrateTo016 = (data = {}, now = Date.now()) => {
  ...
  const items = (migrated.todos?.items || []).map((item, index) => ({
    priority: 'P1', estimatePomos: 1, focusMinutes: null, scheduledStartAt: null, scheduledStatus: 'none', completedPomos: 0, spentMs: 0, done: false, createdAt: now + index, completedAt: null,
    ...item
  }));
